...
function userCtrl($scope) {
	...
	$scope.initializeDB = function(){
		...
		$scope.db.transaction(function(tx){
			...
			tx.executeSql( 'CREATE TABLE IF NOT EXISTS Trip(Id INTEGER PRIMARY KEY AUTOINCREMENT, _license_plate varchar, _cargo varchar, _start_timestamp int, _start_location int, _start_location_formatted varchar,  _start_comments varchar, _end_timestamp int, _end_location int, _end_location_formatted varchar, _end_comments varchar)', [])},
	...
	$scope.AddStartValuesToDB = function() {
		...
		$scope.db.transaction(function(transaction) {
			...
			transaction.executeSql('INSERT INTO Trip(_license_plate, _cargo, _start_timestamp, _start_location, _start_location_formatted, _start_comments) VALUES ("'+$scope.trip.license_plate+'", "'+$scope.trip.cargo+'", "'+$scope.trip.start_timestamp+'", "'+$scope.trip.start_location+'", "'+$scope.trip.start_location_formatted+'", "'+$scope.trip.start_comments+'")');	
	...
	$scope.AddEndValuesToDB = function() {
		...
		// this is the section that actually inserts the values into the User table
		$scope.db.transaction(function(transaction) {
			transaction.executeSql('UPDATE Trip SET _end_timestamp ="'+$scope.trip.end_timestamp+'", _end_location ="'+$scope.trip.end_location+'", _end_location_formatted ="'+$scope.trip.end_location_formatted+'", _end_comments ="'+$scope.trip.end_comments+'" WHERE Id= last_insert_rowid()',[]);
			},function error(err){alert('error on save to local db ' + err)}, function success(){}
		);
		return false;
	} 
